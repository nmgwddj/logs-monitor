...
{
	char* tmpBitsCiphertextAnyLength = new char[data_base_length_*4];
	char* tmpHexCiphertextAnyLength = new char[data_base_length_*2];

	int iLen = 0;

	memset(szCiphertextData,0,data_base_length_);
	memset(tmpHexCiphertextAnyLength,0,data_base_length_*2);
	

	iLen = (((int)strlen(szCipherInBytes)>>2) + ((int)strlen(szCipherInBytes) % 4 == 0 ? 0 : 1))<<4;
	
	memcpy(tmpHexCiphertextAnyLength,szCipherInBytes,strlen(szCipherInBytes));
	Hex2Bits(tmpHexCiphertextAnyLength,tmpBitsCiphertextAnyLength,iLen);
	Bits2Bytes(szCiphertextData,tmpBitsCiphertextAnyLength,iLen);

	delete[] tmpBitsCiphertextAnyLength;
	delete[] tmpHexCiphertextAnyLength;
