...
#include "net/log/global_misc_thread.h"
...
void QLogImpl::DoClearLog()
{
	const long limit_max = 2 * 1024 * 1024, limit_to = 1024 * 1024;
	QLogImpl::GetInstance()->HalfTo(limit_max, limit_to);

	StdClosure task = nbase::Bind(&QLogImpl::DoClearLog);
	net::GlobalMiscThread::GetInstance()->message_loop()->PostDelayedTask(task, nbase::TimeDelta::FromMinutes(10));
}
