...
{
	return NetAddressToString(net_address->ai_addr, (socklen_t)net_address->ai_addrlen);
}
...
std::string NetAddressToStringWithPort(const struct addrinfo* net_address)
{
	return NetAddressToStringWithPort(
		net_address->ai_addr, (socklen_t)net_address->ai_addrlen);
}
...
const uint16_t* GetPortFieldFromAddrinfo(const struct addrinfo* info)
{
	...
	return GetPortFieldFromSockaddr(address, (socklen_t)info->ai_addrlen);
}
