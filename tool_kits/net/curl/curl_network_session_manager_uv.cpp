...
void CurlNetworkSessionManager::AddSession(const SessionScopedRefPtr &session)
{
	...
	if (!session || session->easy_handle_) {
		LOG_WAR(L"0x{0} is NULL or already managed") << (long)session.get();
		return;
	}
	...
	case CURL_POLL_REMOVE:
		if (sockp != nullptr){
			...
			if (response_code_ == 302 || response_code_ == 301){
				...
				LOG_PRO(L"cURL will remove #0x{0}/{1} res:{2}") << (long)session << s << response_code_;
			}
		}
		break;
	}
		...
		if (msg->msg == CURLMSG_DONE) {
			...
			if (finished != nullptr) {
				...
				if (code == CURLE_OK) {
					...
					LOG_PRO(L"#0x{0} completed, {1} running") << (long)finished << still_running_;
				} else {
					DoCheckSessionOrRemoveSafely(finished->shared_from_this());
					LOG_ERR(L"#0x{0} error {1}, {2} running") << (long)finished << code << still_running_;
				}
			}
		}
	}
