...
public:
	...
	* @param[in] replace 是否替换麦克风采集的音频数据,对iOS无效
	* @param[in] cycle 循环的次数，iOS:-1 代表无限循环
	* @param[in] volume 音量[0.0f - 1.0f]
	* @return bool true：方法调用成功 false:方法调用失败
	*/
	bool StartAudioMixing(const char* filePath, bool loopback, bool replace, int cycle, float volume);

	/** @fn bool nim_vchat_pause_audio_mixing();
	* NIM VCHAT DEVICE 停止伴音
	* @return bool true:方法调用成功 false:方法调用失败
	*/
	bool PauseAudioMixing();

	/** @fn bool nim_vchat_resume_audio_mixing();
	* NIM VCHAT DEVICE 语音伴音恢复
	* @return bool true:方法调用成功 false:方法调用失败
	*/
	...
	* @return bool true:方法调用成功 false:方法调用失败
	*/
	bool SetAudioMixingVolume(float volume);
	...
	* @return bool true:方法调用成功 false:方法调用失败
	*/
	bool SetPlayCapturedAudioVolume(float volume);
